...
public class Manager_ClawMovement2 : MonoBehaviour
{
    ...
    public Action clawMachneAction;

    public void AddListener(Action listener)
    {
        clawMachneAction += listener;
    }
    ...
    private void dropClawButtonInput()
    {
            ...
            StageManager.onUseAttempt.Invoke();  // Remove one coin
    ...
    private void clawMove()
    {
        ...
        float clampedZ = Mathf.Clamp(clawHolder.position.z, boundaryZ_Forward, boundaryZ_Back);

        // 제한된 x와 y 좌표를 사용하여 위치 업데이트
        clawHolder.position = new Vector3(clampedX, clawHolder.position.y, clampedZ);
        overHeadMotorRailSystem.position = new Vector3(overHeadMotorRailSystem.position.x, overHeadMotorRailSystem.position.y, clampedZ);
        topMainMotor.position = new Vector3(clampedX, topMainMotor.position.y, clampedZ);

        // Also move the motors
        // overHeadMotorRailSystem.Translate(direction * movementSpeed * 1 * Time.deltaTime);
        // topMainMotor.Translate(direction * movementSpeed * 1 * Time.deltaTime);

        // Move the prop joystick
        propJoyStick.Rotate(direction * movementSpeed * 1 * Time.deltaTime);
    ...
    IEnumerator dropClaw()
    {
        ...
        else
        {
            ...
            if (UnityEngine.Random.Range(1, 10) <= failRate)
            {
    ...
    {
        // Play opening animation
        clawHeadAnimation["Claw_Open_New"].speed = 2f;
    ...
    IEnumerator WeakClaws()
    {
        ...
        yield return new WaitForSeconds(UnityEngine.Random.Range(2.15f, 2.85f));
